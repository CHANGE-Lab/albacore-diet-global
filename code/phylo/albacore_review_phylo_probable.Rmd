...
```{r load prey info data}

#data contains sp list, class, order & family + traits.

#Probable life stage data
my_prey_prob = read.csv(here("data/prey_probable_traits.csv"), header=TRUE) %>%
...
# SAVE TREE ----
write.tree(my_tree_prob, here("data/albacore_diet_tree_prob"))
```

``` {r Load tree}
# Reload tree

my_tree_prob <- read.tree(here("data/albacore_diet_tree_prob"))
...
```{r Check base tree}
#Check tree
mycirc <- ggtree(my_tree_prob, layout = "circular")
#one taxon coming out real strange
mycirc

#checking tree data
str(my_tree_prob)
#301 species parsed through tree.
...
nrow(my_prey_prob) #301
length(my_tree_prob$tip.label) #301
...
x2[4,1] = y2[1]; x2[1,1] = y2[2]; x2[3,1] = y2[3]; x2[5,1] = y2[4];
x2[6,1] = y2[5]; x2[7,1] = y2[7]; x2[9,1] = y2[8]; x2[8,1] = y2[9]
...
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_basic.png'), plot=prob_basic, width=8, height=8, dpi=300)

ggsave(here('./output_figures/diet_phylo/prey_prob/prob_basic_lab.png'), plot=prob_basic_lab, width=8, height=8, dpi=300)
...
prob_class <- ggtree(my_tree_prob_class, aes(color = group), layout = 'circular') +
  scale_colour_manual('Class', aesthetics = c('colour'), values = pal_prob_class,
                      ...
                      labels = c("Actinopterygii", "Cephalopoda","Gastropoda", "Hexanauplia", "Hydrozoa", "Malacostraca","Thaliacea"))## these are the only 3 classes in this dataset
...
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_class.png'), plot=prob_class, width=9, height=9, dpi=300)

ggsave(here('./output_figures/diet_phylo/prey_prob/prob_class_lab.png'), plot=prob_class_lab, width=9, height=9, dpi=300)
...
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_order_lab.png'), plot=prob_order_lab, width=12, height=8, dpi=300)
...
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_fam_lab.png'), plot=prob_fam_lab, width=20, height=8, dpi=300)
...
prob_maxfo <- gheatmap(prob_class, my_prey_prob[,"maxFO", drop = FALSE], 
  ...
  scale_fill_viridis_c(name = "Percent frequency of occurrence in diets (% FO)", na.value = 'grey')

ggsave(here('./output_figures/diet_phylo/prey_prob/prob_maxfo.png'), plot=prob_maxfo, width=10, height=8, dpi=300)
...
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_maxn.png'), plot=prob_maxn, width=10, height=8, dpi=300)
...
prob_maxm <- gheatmap(prob_class, my_prey_prob[,"maxM", drop = FALSE], 
                      offset = 0, width = 0.05, colnames = FALSE) +
  scale_fill_viridis_c(name = "Percent biomass in diets (% kg)", na.value = 'grey')

ggsave(here('./output_figures/diet_phylo/prey_prob/prob_maxm.png'), plot=prob_maxm, width=10, height=8, dpi=300)
...
habitat_traits_prob_final <- gheatmap(habitat_traits_prob3.5, my_prey_prob[ ,'refuge',drop=FALSE], offset=0.15, width=0.05, colnames = F)+
...
#Save vertical graph
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_hab1.png'), plot=habitat_traits_prob1, width=12, height=10, dpi=300)
#save vert + horizontal graph
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_hab2.png'), plot=habitat_traits_prob2, width=12, height=10, dpi=300)
##PROBLEM STARTS HERE
#save vert + horizontal + diel graph
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_hab3.png'), plot=habitat_traits_prob3, width=12, height=10, dpi=300)
#save vert + horizontal + diel graph
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_hab4.png'), plot=habitat_traits_prob_final, width=12, height=10, dpi=300)
...
behav_trophic_prob1 <- gheatmap(prob_basic, my_prey_prob[,'trophic_level', drop=FALSE], 
  ...
  scale_fill_viridis_c(name = "Trophic Level", na.value = 'grey') +
...
behav_trophic_prob2 <- gheatmap(behav_trophic_prob1.5, my_prey_prob[,"gregarious_primary",drop=FALSE], 
                                offset=0.05, width=0.05, colnames = F) +
  scale_fill_viridis_d(name = "Gregariousness", 
                       direction = -1,
...
behav_trophic_prob_final <- gheatmap(behav_trophic_prob2.5, my_prey_prob[ ,'season_migrant',drop=FALSE], offset=0.10, width=0.05, colnames = F) +
...
#Save graph 1
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_behav1.png'), plot=behav_trophic_prob1, width=12, height=10, dpi=300)
#Save graph 2
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_behav2.png'), plot=behav_trophic_prob2, width=12, height=10, dpi=300)
#Save graph 3
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_behav3.png'), plot=behav_trophic_prob_final, width=12, height=10, dpi=300)
...
morphology_comparison_prob1 <- gheatmap(prob_basic, my_prey_prob[ ,'body_shape',drop=FALSE],
...
morphology_comparison_prob2 <- gheatmap(morphology_comparison_prob1.5, my_prey_prob[ ,'b_shape_r',drop=FALSE], 
                                        offset=0.05, width=0.05, colnames = F) +
...
morphology_comparison_prob_final <- gheatmap(morphology_comparison_prob2.5, my_prey_prob[ ,'eye_body_r',drop=FALSE], 
...
#Save graph 1
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_morph1.png'), plot=morphology_comparison_prob1, width=13, height=10, dpi=300)
#Save graph 2
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_morph2.png'), plot=morphology_comparison_prob2, width=13, height=10, dpi=300)
#Save graph 3
ggsave(here('./output_figures/diet_phylo/prey_prob/prob_morph3.png'), plot=morphology_comparison_prob_final, width=13, height=10, dpi=300)
