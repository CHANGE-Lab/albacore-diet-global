...
alb_global_diet_total = read.csv(here::here("data/output_data/alb_global_diet_total.csv"))
...
```{r Summary - wide diet pa data, tidy=TRUE, message=FALSE}

alb_global_wide_dietpa_sites = read.csv(here("data/output_data/alb_global_wide_dietpa.csv")) %>%
  dplyr::select(StudyID:lat_cat)

alb_global_wide_dietpa = read.csv(here("data/output_data/alb_global_wide_dietpa.csv")) %>%
  dplyr::select(-c(StudyID:lat_cat))

#The number of observations (rows) and species (columns)
dim(alb_global_wide_dietpa)
```

```{r Summary - wide diet fo data, tidy=TRUE, message=FALSE}

alb_global_wide_dietfo_sites = read.csv(here("data/output_data/alb_global_wide_dietfo.csv")) %>%
  dplyr::select(StudyID:lat_cat)
...
print(paste0("(i) percent frequency of occurrence for ", ncol(diet_fo[,9:ncol(diet_fo)])))

# % Numerical data

diet_num = read.csv(here("data/output_data/alb_global_wide_dietn.csv"))

print(ncol(diet_fo[,9:ncol(diet_num)]))

# % Biomass data

diet_mass = read.csv(here("data/output_data/alb_global_wide_dietm.csv"))

print(ncol(diet_fo[,9:ncol(diet_mass)]))
```
