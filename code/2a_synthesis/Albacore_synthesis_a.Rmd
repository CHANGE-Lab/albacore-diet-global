...
#Delete the second of each of these
  slice(., -c(954, 119, 687)) %>%
  dplyr::select(StudyID, OceanBasin, OceanBasinQ, LocatName, LocatLatitude, LocatLongitude, YearEnd, pred_life, pred_flmean, prey_sp, presence) %>%
...
# data
PreyPAdata = as.data.frame(alb_global[,11:ncol(alb_global)])
PreyPAsite = as.data.frame(alb_global[,c(1,2,7)])
