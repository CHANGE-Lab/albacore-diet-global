...
which(colSums(alb_prey_pa[,10:ncol(alb_prey_pa)])==0) #Remove

names(which(colSums(alb_prey_pa[,10:ncol(alb_prey_pa)])==0))

alb_prey_use = alb_prey_pa[colnames(alb_prey_pa) %notin% names(which(colSums(alb_prey_pa[,10:ncol(alb_prey_pa)])==0))]

#Check for empty rows
which(rowSums(alb_prey_use[,10:ncol(alb_prey_use)])==0) #Remove these - this becomes a thing once we start removing outlier species
#None

#Subset species df
alb_prey_use = alb_prey_use[,10:ncol(alb_prey_use)]
names(alb_prey_use) <- str_replace_all(names(alb_prey_use), " ", "_")

#check df
dim(alb_prey_use) #236 species & 65 observations
#str(alb_adult_prey) #data are integers & numeric

#Try numeric
#alb_prey_use[,1:ncol(alb_prey_use)] =  lapply(alb_prey_use[,1:ncol(alb_prey_use)], as.numeric)
...
alb_prob_clust = read.csv(paste(here("./data/output_data/prob.prey.clusternum.divis.k6.hab.csv"), 
...
alb_trait_use = alb_trait_use %>%
  filter(prey_sp %in% names(alb_prey_use))
...
alb_prey_use <- alb_prey_use[colnames(alb_prey_use) %in% rownames(alb_trait_use)]

dim(alb_prey_use) #70 rows and 226 spp

```

Now we need to check for and remove any zero-sum columns or rows of data, and adjust the co-variate data.

```{r Diet data check zero sum, tidy=TRUE, warning=FALSE, message=FALSE}

#Need to check for empty columns and rows again
#Check for empty columns
which(colSums(alb_prey_use)==0) #None! #Otherwise remove these
#Check for empty rows
which(rowSums(alb_prey_use)==0) #Remove these - this becomes a thing once we start removing outlier species

```

```{r Diet data check less than two occurrences, tidy=TRUE, warning=FALSE, message=FALSE}

#Need to check for empty columns and rows again
#Check for empty columns
which(colSums(alb_prey_use)<2) #Consider removing
length(which(colSums(alb_prey_use)<2)) #Consider removing #91 spp with low occurrence
#Check for empty rows
which(rowSums(alb_prey_use)<2) #Consider removing
...
dim(alb_trait_use) #226 species and 8 traits
dim(alb_prey_use) #65 obs, 226 species
dim(alb_covars) #65 obs, 9 covariates
...
#L - species df
write.csv(alb_prey_use, here("./data/output_data/alb_prey_pa_use.csv"))

#R - covars df
write.csv(alb_covars, here("./data/output_data/alb_covars_use.csv"))

#Q - trait df
write.csv(alb_trait_use, here("./data/output_data/alb_trait_use.csv"))
