...
#This no longer works with retirement of rgdal
#longs <- readOGR("code/longhurst/Longhurst_world_v4_2010.shp")
#This works
#longs <- st_read("code/longhurst/Longhurst_world_v4_2010.shp")
#Also this and same code as above
longs <- sf::read_sf(here::here("code/longhurst/Longhurst_world_v4_2010.shp")) 
#check the spatial polygon df
longs 
head(st_coordinates(longs))
st_crs(longs)
```
...
slot(alb, "proj4string") <- crs_wgs84 #sf::st_crs(longs)
sf::st_crs(alb) == sf::st_crs(longs)

#alb df does not have a CRS.
#Not working without building new df in code chunk below, but then there's an issue with that df not being an spdf.
#st_crs(alb) <- st_crs(longs)
...
provinces <- rgeos::overGeomGeom(alb, longs)

```

```{r more spatial df manip}
alb2 <- sf::st_as_sf(alb, coords = c("X", "Y"))
alb2
sf::st_crs(alb2) <- sf::st_crs(longs)
sf::st_crs(alb2)

#install.packages("remotes")
#remotes::install_github("laurimyllyvirta/lauR")
#library(lauR)
#alb3 <- spdf(alb2, crs = NULL, llcols = NULL, na.action = na.omit)

provinces <- sp::over(alb2, longs)
alb$province <- provinces$ProvDescr
alb$code <- provinces$ProvCode

#sp code not working 
coordinates(alb2)
provinces <- rgeos::overGeomGeom(alb2, longs)

```

```{r alternative import alb covars data and manip}
...
# Base map
world_map <- rnaturalearth::ne_countries(scale = 'small', returnclass = c("sf"))
baseMap <- ggplot() + geom_sf(data = world_map, size = .2, fill = "gray80", col = "gray90") +
  theme(panel.grid.major = element_line(color = gray(0.9), linetype = "dashed", size = 0.5))
baseMap

# Load the Longhurst shapefile, convert to SpatialPolygonsDataFrame
longhurst <- st_read("Longhurst_world_v4_2010.shp") 
...
# Load points
alb <- read.csv("2_output_data/alb_covars.csv",  head = TRUE, sep= ",")
...
# Map
albMap <- longMap + geom_point(data = as.data.frame(alb), aes(x = LocatLongitude, y = LocatLatitude)) + ylim(c(-55, 63))
albMap
