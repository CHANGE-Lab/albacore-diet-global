...
alb_prey_fo = read.csv(paste(here::here("data/output_data/alb_global_wide_dietfo.csv"),
...
print(dim(alb_prey_fo)) #45 240 from 70 obs after removing the other ocean basins noted above
```
...
which(colSums(alb_prey_fo[,10:ncol(alb_prey_fo)])<2) #Remove

length(which(colSums(alb_prey_fo[,10:ncol(alb_prey_fo)])<2)) #173 species have low prevalence

names(which(colSums(alb_prey_fo[,10:ncol(alb_prey_fo)])<2))

alb_prey_use = alb_prey_fo[colnames(alb_prey_fo) %notin% names(which(colSums(alb_prey_fo[,10:ncol(alb_prey_fo)])<2))]
...
alb_prob_clust = read.csv(paste(here::here("./data/output_data/prob.prey.clusternum.divis.k7.csv"), 
                                sep=""), header=T, row.names = 1) %>%
   dplyr::select(prey_sp:prob.clust.num)

dim(alb_prob_clust) #we obtain 292 species, 11 traits or covariates

add_order = read.csv(paste(here::here("./data/output_data/prey_trait_select.csv"))) %>%
  select(prey_sp, prey_order)

alb_trait_fo_use = alb_prob_clust %>%
  left_join(add_order) %>%
...
alb_trait_fo_use$prey_sp = str_replace_all(alb_trait_fo_use$prey_sp, " ", "_")
...
alb_trait_fo_use[,3:8] <- lapply(alb_trait_fo_use[,3:8], factor) #to convert to factors

#Need to filter by prey species we want to use.
alb_trait_fo_use = alb_trait_fo_use %>%
  filter(prey_sp %in% names(alb_prey_use_spp))
  #we retain 292 species and 8 trait values

dim(alb_trait_fo_use) #141 spp 9 traits
str(alb_trait_fo_use)
...
summary(alb_trait_fo_use$life_stage) #low numbers for larva, do not use

summary(alb_trait_fo_use$vert_habitat) #use

summary(alb_trait_fo_use$horz_habitat) #use

summary(alb_trait_fo_use$diel_migrant_cat) #a lot more diel migrants than not

summary(alb_trait_fo_use$season_cat) #more seasonal taxa than not

summary(alb_trait_fo_use$gregarious) #still more gregarious than not, just an artefact of the pelagic prey data.

summary(as.factor(alb_trait_fo_use$prob.clust.num)) #good distribution of species within clusters
...
alb_prey_fo_use <- alb_prey_use_spp[colnames(alb_prey_use_spp) %in% rownames(alb_trait_fo_use)]

dim(alb_prey_use) #45 rows and 155 spp
...
dim(alb_trait_fo_use) #141 species and 9 traits
dim(alb_prey_fo_use) #45 obs, 141 species
dim(alb_covars) #45 obs, 9 covariates
...
#PA
check_nmds = metaMDS(alb_prey_fo_use,
                     distance="bray",
...
#L - species df
write.csv(alb_prey_fo_use, here::here("./data/output_data/alb_prey_fo_use.csv"), row.names=FALSE)

#R - covars df
write.csv(alb_covars, here::here("./data/output_data/alb_covars_use.csv"), row.names=FALSE)

#Q - trait df
write.csv(alb_trait_fo_use, here::here("./data/output_data/alb_trait_fo_use.csv"), row.names=FALSE)
