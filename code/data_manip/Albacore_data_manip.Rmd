---
title: "Albacore Review Data Manipulation & Graphs"
...
## Albacore Review Dataframes

### Prey morphometric measuremetns
We first want to load in and clean the prey morphometric measurements, check data and calculate eye to body and body shape ratios, and summarise to give an average value for each species for which we obtained images. We are able to obtain values for 293 species.

```{r Prey Morph Data Clean}

#Load prey morphometric measurement data
prey_morph <- read.csv(here("data/prey_morphometrics.csv"), header=TRUE) 
#Issues reading in the data, everything was read in as a factor, 
#Can use the following argument when reading in:
#stringsAsFactors = FALSE
#I'm going to read in as is, then convert to character --> numeric.
str(prey_morph)
#glimpse(prey_morph)
#names(prey_morph[,12:17])
...
#Check again
str(prey_morph)
#prey_morph[,"s_length":"eye_diameter"] <- as.numeric(prey_morph[, "s_length":"eye_diameter"]) #Does not appear to take the column names

#Additional data checks
dim(prey_morph) #1290 rows and 21 columns
names(prey_morph)
unique(prey_morph$image_type)
