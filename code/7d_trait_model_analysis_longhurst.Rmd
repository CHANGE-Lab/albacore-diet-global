...
This document contains code for loading mostly manipulated data for global albacore prey communities consumed and for conducting multivariate community analyses and traitglm on albacore diets across selected Longhurst Province codes, using habitat association, gregariousness traits and cluster # output from cluster analyses.

This change to the previous models based on the three most sampled Ocean Basins (N Atlantic, Mediterranean and N Pacific) addresses a key issue in the post hoc sampling design, whereby most data points within an Ocean are often concentrated from a single biogeographic region within that ocean. As the meta-data were associated with Longhurst Province Codes, and we selected those with enough replication within the three aforementioned Ocean Basins, these were: CCAL (California Current), MEDI (Mediterranean Sea), NADR (North Atlantic Drift).
...
For Longhurst Province Code analysis:

- Species (L): alb_prey_use_longhurst ==> "./data/2_output_data/alb_prey_pa_use_longhurst.csv"

- Env/Site (R): alb_covars_longhurst ==> "./data/2_output_data/alb_covars_longhurst.csv"

- Traits (Q): alb_trait_use_longhurst ==> "./data/2_output_data/alb_trait_use_longhurst.csv"
...
summary(alb_trait_use$gregarious) #DON'T USE
...
ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_clusters_many_graph.pdf"), 
       plot = as.ggplot(ocean_clusters_many_graph), width=12, height=12, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_clusters_many_graph.jpeg"), 
       plot = as.ggplot(ocean_clusters_many_graph), width=12, height=12, dpi=300)
...
ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_cluster_LASSO_graph.pdf"), 
       plot = as.ggplot(ocean_cluster_LASSO_graph), width=8, height=8, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_cluster_LASSO_graph.jpeg"), 
       plot = as.ggplot(ocean_cluster_LASSO_graph), width=8, height=8, dpi=300)
...
ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_phylo_many_graph.pdf"), 
       plot = as.ggplot(ocean_phylo_many_graph), width=12, height=18, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_phylo_many_graph.jpeg"), 
       plot = as.ggplot(ocean_phylo_many_graph), width=12, height=18, dpi=300)
...
colnames(temp) = c("Amphipoda", "Aulopiformes", "Beloniformes", "Caenogastropoda",
                  ...
                  "Ophidiiformes", "Osmeriformes", "Perciformes", "Pleuronectiformes",
                  "Pteropoda", "Scorpaeniformes", "Sepiida", "Siphonophorae",
                  "Stephanoberyciformes", "Stomiiformes", "Syngnathiformes", "Vampyromorpha") 
...
ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_phylo_LASSO_graph.pdf"), 
       plot = as.ggplot(ocean_phylo_LASSO_graph), width=12, height=12, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_phylo_LASSO_graph.jpeg"), 
       plot = as.ggplot(ocean_phylo_LASSO_graph), width=12, height=12, dpi=300)
...
ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_traits_many_graph.pdf"), 
       plot = as.ggplot(ocean_trait_many_graph), width=12, height=12, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_traits_many_graph.jpeg"), 
       plot = as.ggplot(ocean_trait_many_graph), width=12, height=12, dpi=300)
...
ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_traits_LASSO_graph.pdf"), 
       plot = as.ggplot(ocean_LASSO_graph), width=12, height=8, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022/ocean_traits_LASSO_graph.jpeg"), 
       plot = as.ggplot(ocean_LASSO_graph), width=12, height=8, dpi=300)
...
### Phylogeny + Traits models

**MANYGLM Phylo + Trait - Ocean Basin**

Trait (phylo)*Env (ocean basin) - Manyglm

```{r Phylo ocean manyglm}

ocean_phylo_trait_many <- traitglm(L = alb_prey_pa_use_mv,
                                     R = alb_covars_use$OceanBasin,
                                     Q = alb_trait_use[,c(2,4:7)],
                                     family=binomial(link = "logit")
                                     ) #note default is "manyglm"
...
a        = max( abs(ocean_phylo_trait_many$fourth.corner) )
colort   = colorRampPalette(c("blue","grey","red")) #can replace "grey" with "white"
temp = t(as.matrix(ocean_phylo_trait_many$fourth.corner))
...
ocean_phylo_trait_many_graph = levelplot(temp, 
                     xlab=list("Ocean Basins", cex=1.5),
                     ylab=list("Species phylogeny", cex=1.5), 
                     col.regions=colort(100), 
                     at=seq(-a, a,length=100),
...
print(ocean_phylo_trait_many_graph)
...
ocean_phylo_trait_LASSO <- traitglm(L = alb_prey_pa_use_mv,
                              R = alb_covars_use$OceanBasin,
                              Q = alb_trait_use[,c(2,4:7)],
                              family=binomial(link = "logit"),
                              method="glm1path"
...
a        = max( abs(ocean_phylo_trait_LASSO$fourth.corner) )
colort   = colorRampPalette(c("blue","grey","red")) #can replace "grey" with "white"
temp = t(as.matrix(ocean_phylo_trait_LASSO$fourth.corner))
...
ocean_phylo_trait_LASSO_graph = levelplot(temp, 
                     xlab=list("", cex=1.5), #Ocean Basins
                     ylab=list("Phylogenetic Order", cex=1.5), 
                     col.regions=colort(100), 
                     at=seq(-a, a,length=100),
...
print(ocean_phylo_trait_LASSO_graph)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022_PA/ocean_phylo_trait_LASSO.pdf"), 
       plot = as.ggplot(ocean_phylo_trait_LASSO), width=12, height=24, dpi=300)

ggsave(here::here("outputs_figures/traitglms/ocean_models_2022_PA/ocean_phylo_trait_LASSO.jpeg"), 
       plot = as.ggplot(ocean_phylo_trait_LASSO), width=12, height=24, dpi=300)

ocean_phylo_trait_LASSO$deviance
...
saveRDS(ocean_phylo_many, file = here::here("outputs_figures/traitglms/ocean_models_2022_PA/ocean_phylo_many.rds"))
...
saveRDS(ocean_phylo_LASSO, file = here::here("outputs_figures/traitglms/ocean_models_2022_PA/ocean_phylo_LASSO.rds"))
saveRDS(ocean_trait_LASSO, file = here::here("outputs_figures/traitglms/ocean_models_2022_PA/ocean_trait_LASSO.rds"))
saveRDS(ocean_phylo_trait_LASSO, file = here::here("outputs_figures/traitglms/ocean_models_2022_PA/ocean_phylo_trait_LASSO.rds"))
