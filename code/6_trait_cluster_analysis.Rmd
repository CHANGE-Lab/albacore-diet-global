...
- prey_probable_load == prey_traits_loads
- prey_probable == prey_traits
- probable_species == prey_species
...
use_traits[,1:6] = lapply(use_traits[,1:6], as.factor)


#Old version didn't include gregarious
#Now we have 292 species instead of ~137.
...
Below we are using simplified habitat use trait values + gregarious (as binary) traits for: (i) divisive and
(ii) agglomerative (complete) clustering techniques. We assess primarily: 
...
write.csv(prob.stats.df.divisive, here("outputs_figures/clusters/prob_cluster_mar2022/prob.stats.df.divisive.csv"))
...
write.csv(prob.stats.df.aggl, here("outputs_figures/clusters/prob_cluster_mar2022/prob.stats.df.aggl.csv"))
...
* Elbow method (29/03/2022) using habitat use + gregarious (binary) + life stage inflection at 6

* Silhouette (29/03/2022) using habitat use + gregarious (binary) + life stage inflection at 6

* Other metrics such as average between, cluster balance, and dunny smith residuals support k = 6.
...
prob.aggl.elbow = ggplot(data = data.frame(t(cstats.table(prob.gower.dist, prob.aggl.clustc, 20))), 
...
prob.aggl.elbow
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.elbow.png'), 
       plot=prob.aggl.elbow, width=8, height=8, dpi=300)
...
# Agglomerative
prob.aggl.silh = ggplot(data = data.frame(t(cstats.table(prob.gower.dist, prob.aggl.clustc, 20))), 
  ...
  theme(plot.title = element_text(hjust = 0.5))
prob.aggl.silh
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.silh.png'), 
       plot=prob.aggl.silh, width=8, height=8, dpi=300)
```
...
prob.div.elbow = ggplot(data = data.frame(t(cstats.table(prob.gower.dist, prob.divisive.clust, 20))), 
  ...
  theme(plot.title = element_text(hjust = 0.5))
prob.div.elbow
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.div.elbow.png'), 
...
prob.div.silh = ggplot(data = data.frame(t(cstats.table(prob.gower.dist, prob.divisive.clust, 20))), 
  ...
  theme(plot.title = element_text(hjust = 0.5))
prob.div.silh
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.div.silh.png'), 
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.dendro.horz.k7.png'), plot=prob.dendro.graph, width=8, height=8, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.dendro.rad.k7.png'), plot=prob.dendro.rad, width=8, height=8, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.dendro.vertlabs.k7.pdf'), plot=ggd1, width=5, height=20, dpi=300)
...
prob.dendro.vert 
#Export as .png
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.dendro.vert2.k7.png'), plot=prob.dendro.vert, width=5, height=12, dpi=300)
...
write.csv(prob.prey.cl, here("outputs_figures/clusters/prob_cluster_mar2022/prob.prey.clusternum.divis.k7.csv"))
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.dendro.heatcounts.divis.k7.life.pdf'), plot=heatmap.c, width=8, height=12, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.dendro.heatpercent.divis.k7.life.pdf'), plot=heatmap.p, width=8, height=12, dpi=300)
...
ggsave(here("outputs_figures/clusters/prob_cluster_mar2022/trait_nMDS_prob_clusters_div.png"), 
...
prob.dendro <- as.dendrogram(prob.aggl.clustc) #292 species
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.dendro.horz.k6.png'), plot=prob.dendro.graph, width=8, height=8, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.dendro.rad.k6.png'), plot=prob.dendro.rad, width=8, height=8, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.dendro.vertlabs.k6.pdf'), plot=ggd1, width=5, height=20, dpi=300)
...
prob.dendro.vert 
#Export as .png
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.dendro.vert2.k6.png'), plot=prob.dendro.vert, width=5, height=12, dpi=300)
...
#Extract cluster number to trait matrix
prob.clust.num.aggl <- cutree(prob.aggl.clustc, k = 6)

#we want to bind the original dataset with the cluster numbers such that each species is assigned a cluster
#can use whole data or just traits use to just look at unique species clusters in relation to traits
#alb.cl <- cbind(ctraitsO, alb.clust.num)
#OR
prob.prey.cl.aggl <- cbind(prey_traits, prob.clust.num.aggl)

#View(prob.prey.cl)

write.csv(prob.prey.cl.aggl, 
          here("outputs_figures/clusters/prob_cluster_mar2022/prob.prey.clusternum.aggl.k6.csv"))
...
#Create dfs for graphs
prob.clust.long.aggl = prob.prey.cl.aggl %>%
  dplyr::select(prey_sp, life_stage:season_cat, `gregarious`, `prob.clust.num.aggl`) %>% #maxFO:maxM, 
  reshape2::melt(id.vars = c("prey_sp", "prob.clust.num.aggl"), variable.name = "trait", value.name = "level") %>%
  dplyr::rename(prob.clust.num = prob.clust.num.aggl) %>%
  group_by(prob.clust.num, trait, level) %>%
  mutate(count = n_distinct(prey_sp)) %>%
  distinct(prob.clust.num, trait, level, count) %>% #, percent
  group_by(prob.clust.num, trait) %>%
...
#heatmap.c will be suitable in case you want to go for absolute counts - but it doesn't tell much to my taste
#problem below involves the values of our data being ordinal, therefore they are not unique
levels(prob.clust.long.aggl$trait)

#Our data above comes truncated, you would need to truncate the data and re-label clusters depending on which dfs you melt/merge/reshape.
#Example: View(alb.cust.long.q[96:nrow(alb.cust.long.q),])
heatmap.c <- ggplot(prob.clust.long.aggl, aes(x = factor(prob.clust.num), y = level)) +
  geom_tile(aes(fill = count))+
  labs(title = "Distribution of characteristics across clusters by counts", x = "Cluster number", y = NULL) +
  scale_fill_viridis(option="magma", begin = 0.2, end = 0.95)+
  facet_grid(trait~. , scales="free_y")
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.dendro.heatcounts.k6.life.pdf'), plot=heatmap.c, width=8, height=12, dpi=300)


heatmap.p <- ggplot(prob.clust.long.aggl, aes(x = factor(prob.clust.num), y = factor(level, ordered = T))) +
  geom_tile(aes(fill = percent), alpha = 0.85)+
  labs(title = "Distribution of characteristics across clusters by percentage", x = "Cluster number", y = NULL) +
  #scale_fill_gradient2(low = "darkslategray1", mid = "yellow", high = "turquoise4") +
  scale_fill_viridis(option="magma", begin = 0.2, end = 0.95)+
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/prob.aggl.dendro.heatpercent.k6.life.pdf'), plot=heatmap.p, width=8, height=12, dpi=300)

```

```{r Summary info, warning=FALSE, message=FALSE}
summary(as.factor(prob.prey.cl.aggl$prob.clust.num))
#   1   2   3   4   5   6 
#132  65  49  26  15   5 
...
## NMDS for checking on our ordination - agglomerative
...
#bind treatment labels and score values
treatment.scores <- cbind(prob.prey.cl.aggl, data.scores)
...
```{r Convex hulls by cluster number aggl, warning=FALSE, message=FALSE}
#Create convex hulls for the space occupied by each cluster value
unique(treatment.scores$prob.clust.num.aggl)
#length(treatment.scores$prob.clust.num)

#Cluster number - hull loop
clust = as.character(unique(treatment.scores$prob.clust.num.aggl))
for(i in 1:length(clust)) {
  temp = clust[i]
  df = treatment.scores[treatment.scores$prob.clust.num.aggl == temp, ][chull(treatment.scores[treatment.scores$prob.clust.num.aggl == temp, c("NMDS1", "NMDS2")]), ]
  assign(paste0('grp.',temp), df)
}

#combine the hull data
hull.data2 <- rbind(grp.1, grp.2, grp.3, grp.4, grp.5, grp.6)  
...
trait_nMDS_prob_fig2 <- ggplot() + 
  geom_polygon(data=hull.data2, 
               aes(x=NMDS1, y=NMDS2, 
                   fill= prob.clust.num.aggl, 
                   group= prob.clust.num.aggl), 
               alpha=0.30) + # add the convex hulls
  geom_point(data=treatment.scores, 
             aes(x=NMDS1, y=NMDS2, colour= prob.clust.num.aggl), 
             size=2) + # add the point markers
  coord_equal() +
  theme_bw()  +
  theme(axis.text.x = element_blank(),  # remove x-axis text
...
trait_nMDS_prob_fig2

ggsave(here("outputs_figures/clusters/prob_cluster_mar2022/trait_nMDS_prob_clusters_aggl.png"), 
       plot = trait_nMDS_prob_fig2, width = 8, height = 8, dpi = 300)
