...
- We therefore removed 206 species with low prevalence (occurs < 3 times), under code chunk 4.

- The data are longer (57 obs) than they are wide (98 species).
...
```{r Species PA (L) load data for oceans, tidy=TRUE, warning=FALSE, message=FALSE}
...
```{r Species PA (L) remove low abundance taxa for oceans, tidy=TRUE, warning=FALSE, message=FALSE}
#Check for empty columns
which(colSums(alb_prey_pa[,10:ncol(alb_prey_pa)])<2) #Remove
length(which(colSums(alb_prey_pa[,10:ncol(alb_prey_pa)])<3)) #206 species have low prevalence
...
```{r Select species only (L) for oceans, tidy=TRUE, warning=FALSE, message=FALSE}
#Subset species df
...
#check df
dim(alb_prey_use_spp) #98 species & 57 observations
...
```{r Trait DF (Q) inc clust num for oceans, tidy=TRUE, warning=FALSE, message=FALSE}
...
```{r Traits with phylogenetic order info}
...
```{r Manip trait DF (Q) for oceans, tidy=TRUE, warning=FALSE, message=FALSE}
...
alb_trait_use[,4:9] <- lapply(alb_trait_use[,4:9], factor) #to convert to factors
...
```{r Trait values check for oceans, warning=FALSE, message=FALSE}
...
```{r FILTER PREY BY PROB TRAITS for ocean basins, tidy=TRUE, warning=FALSE, message=FALSE}
...
```{r Covars oceans (R) checking, eval=FALSE}
...
unique(alb_covars$StudyID)
```

## DF checks

```{r Check names and df dimensions for ocean basins, eval=FALSE}
...
```{r Species PA (L) load data filter for longhurst use, tidy=TRUE, warning=FALSE, message=FALSE}
...
```{r Species PA (L) remove low abundance taxa for longhurst use, tidy=TRUE, warning=FALSE, message=FALSE}
#Check for empty columns
...
```{r Select species only (L) for longhurst use, tidy=TRUE, warning=FALSE, message=FALSE}
#Subset species df
...
```{r Manip trait DF (Q) for longhurst use, tidy=TRUE, warning=FALSE, message=FALSE}
#Relabel to add underscore in names
...
```{r Trait values check for longhurst use, warning=FALSE, message=FALSE}
...
```{r FILTER PREY BY PROB Longhurst data, tidy=TRUE, warning=FALSE, message=FALSE}
...
```{r Covars longhurst (R) checking, eval=FALSE}
#Check representation
#summary(alb_adult_covars)
...
```{r Saving ocean dfs for analysis}
...
```{r Saving longhurst dfs for analysis}
