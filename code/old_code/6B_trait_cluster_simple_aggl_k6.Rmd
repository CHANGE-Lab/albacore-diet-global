...
use_traits = prey_traits %>%
  dplyr::select(vert_habitat, horz_habitat, diel_migrant_cat, season_cat, gregarious) #life_stage, 
...
Here we generate a multivariate species-based dissimilarity matrix of the data, and a hierarchical agglomerative clustering algorithm with complete linkage.
...
Ultimately we are aiming for distinct clusters of species, such that the difference within clusters is minimal and between clusters is maximised. Assessing cluster statistical tables, we are consistently observing higher average.within cluster differences using agglomerative clustering compared to divisive algorithms. We have selected the divisive method and this is described in another code doc.

Below we are using simplified habitat use trait values + gregarious (as binary) traits for:  agglomerative (complete) clustering techniques. We assess primarily: 
...
**NOTE: We observe the greatest balance between these cluster validation criteria for the hierarchical agglomerative clustering algorithm and for k = 6 clusters.**
...
write.csv(prob.stats.df.aggl, here("outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.stats.df.aggl.csv"))
...
prob.aggl.elbow
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.elbow.png'), 
...
prob.aggl.silh = ggplot(data = data.frame(t(cstats.table(prob.gower.dist, prob.aggl.clustc, 20))), 
  ...
  theme(plot.title = element_text(hjust = 0.5))
prob.aggl.silh
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.silh.png'), 
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.dendro.horz.k6.png'), plot=prob.dendro.graph, width=8, height=8, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.dendro.vertlabs.k6.pdf'), plot=ggd1, width=5, height=20, dpi=300)
...
prob.dendro.vert 
#Export as .png
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.dendro.vert2.k6.png'), plot=prob.dendro.vert, width=5, height=12, dpi=300)
...
write.csv(prob.prey.cl.aggl, 
          here("outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.prey.clusternum.aggl.k6.csv"))
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.dendro.heatcounts.k6.life.pdf'), plot=heatmap.c, width=8, height=12, dpi=300)
...
ggsave(here('outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/prob.aggl.dendro.heatpercent.k6.life.pdf'), plot=heatmap.p, width=8, height=12, dpi=300)
...
ggsave(here("outputs_figures/clusters/prob_cluster_mar2022/aggl_hab_agg/trait_nMDS_prob_clusters_aggl.png"), 
