...
```{r Workspace, include=FALSE, echo=FALSE, message=FALSE, results='hide'}
...
```{r Species FO (L) load data, warning=FALSE, message=FALSE}
...
```{r Species FO (L) manipulate, warning=FALSE, message=FALSE}
...
```{r Species PA (L), warning=FALSE, message=FALSE}
...
```{r Trait DF (Q) - prob clust num, warning=FALSE, message=FALSE}
...
```{r TraitGLM - SPP - life manyglm binom, eval=FALSE}
...
```{r TraitGLM - SPP - ocean manyglm binom, eval=FALSE}
...
```{r TraitGLM - SPP - lat manyglm binom, eval=FALSE}
...
*Note:* that the .Rmd doc also contains additional code to evaluate these models and for plotting the species models.

```{r EXTRA code for model eval, eval=FALSE, include=FALSE}
...
```{r TraitGLM life clusters manyglm binom, eval=FALSE}
...
```{r Plot 4th corner life many, eval=FALSE}
...
```{r TraitGLM life clusters LASSO, eval=FALSE}
...
Call:  traitglm(L = alb_prob_preyPA_mv, R = alb_prob_covars$pred_life,      Q = traits_prob$cluster, family = binomial(link = "logit")) 
[1] "binomial(link=logit)"
...
Call:  traitglm(L = alb_prob_preyPA_mv, R = alb_prob_covars$pred_life,      Q = traits_prob[, 1:6], family = binomial(link = "logit")) 
[1] "binomial(link=logit)"
...
Call:  traitglm(L = alb_prob_preyPA_mv, R = alb_prob_covars$OceanBasin,      Q = traits_prob[, 1:6], family = binomial(link = "logit")) 
[1] "binomial(link=logit)"
