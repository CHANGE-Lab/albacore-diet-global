...
The following summarises the max observed FO, N and M for each species in the dataset AND for each life stage likely consumed. Note that there are 28 taxa for which two stages were consumed. In the chunk below this one we will select the prey species and life stage combination that was most common. Most of the time this wouldn't be a large problem because typically one life stage was consumed much more frequently than the other. However, for *Cololabis saira*, the %FO of the adult and juvenile life stages consumed were very similar in summary with the adult life stage slightly higher. 
...
alb_global_prob_metrics = alb_global_prob %>%
  dplyr::select(prey_class:prey_sp, prey_age_reported_1, life_stage, pFOuse, pN, pM, maxtl_use, gape_lmax, prey_length) %>%
  group_by(prey_sp, prey_age_reported_1, life_stage) %>%
  dplyr::summarize(maxFO = max(pFOuse, na.rm=TRUE), maxN = max(pN, na.rm=TRUE), maxM = max(pM, na.rm=TRUE),
                   maxTL = max(maxtl_use, na.rm=TRUE)/10, meanGAPE = mean(gape_lmax, na.rm=TRUE), meanLENGTH = mean(prey_length, na.rm=TRUE))
...
write.csv(alb_global_prob_metrics2, here("data/output_data/alb_global_prob_metrics"), row.names = FALSE)
