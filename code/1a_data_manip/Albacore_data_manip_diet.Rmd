...
```{r Load trait dataframes, tidy = TRUE}
...
```{r Albacore diet data + probable life stage estimation, tidy = TRUE}
...
```{r Alb probable diet metrics summary, tidy = TRUE}

#All combinations of life stages and species consumed
alb_global_prob_metrics = alb_global_prob %>%
  dplyr::select(prey_class:prey_sp, prey_age_reported_1, life_stage, pFOuse, pN, 
                pM, maxtl_use, l_adult, gape_lmax, 
  ...
  dplyr::summarize(maxFO = max(pFOuse, na.rm=TRUE), maxN = max(pN, na.rm=TRUE), 
                   maxM = max(pM, na.rm=TRUE), maxTL = max(maxtl_use, na.rm=TRUE)/10, 
                   meanGAPE = mean(gape_lmax, na.rm=TRUE), 
                   meanLENGTH = mean(prey_length, na.rm=TRUE), 
                   meanFRAC = mean(prey_frac, na.rm=TRUE)) %>%
...
```{r Albacore prey life stages lists, tidy = TRUE}
...
```{r Extract information when albacore prey size was reported, tidy = TRUE}

alb_prey_raw_size = alb_global %>%
  filter(maxtl_use > 0) %>%
  dplyr::select(StudyID, OceanBasin, LocatName, LocatLatitude, LocatLongitude, 
                YearEnd, pred_life, pred_flmean, pred_flmin, pred_flmax, 
                gape_lmax, prey_sp, maxl_use, maxl_type, standard_total, maxtl_use, 
                pFOuse, pN, pM) %>%
  mutate(maxtl_use = maxtl_use/10,
         maxl_use = maxl_use/10,
         presence = 1)

write.csv(alb_prey_raw_size, here("data/output_data/alb_prey_raw_size.csv"), 
          row.names = FALSE)
...
```{r Prey adult traits + morph + qual data, tidy = TRUE}
...
#Save
write.csv(prey_adults_merge, here("data/output_data/prey_traits_adults.csv"), 
          row.names = FALSE)
...
```{r Prey probable list + traits + morph + qual data, tidy = TRUE}
...
prey_probable_merge = alb_global_prob_metrics %>%
  ...
  dplyr::select(prey_class:prey_family, prey_sp, prey_age_reported_1, life_stage, 
                ...
                b_shape_r:standard_total, phys_defense:countershade, 
                energy_density:percent_lipid, maxFO:maxM, maxTL, trophic_level, 
                fisheries_status)

#303 spp and ~28 traits
glimpse(prey_probable_merge)

write.csv(prey_probable_merge, here("data/output_data/prey_trait_probable_all.csv"), 
          row.names = FALSE)
...
```{r Prey probable list + traits + morph + qual data for selected life stage, tidy = TRUE}
...
prey_probable_merge2 = alb_global_prob_metrics2 %>%
  ...
  dplyr::select(prey_class:prey_family, prey_sp, prey_age_reported_1, life_stage, 
                ...
                b_shape_r:standard_total, phys_defense:countershade, 
                energy_density:percent_lipid, maxFO:maxM, maxTL, trophic_level, 
                fisheries_status) #meanGAPE, meanLENGTH, meanFRAC,

#308 spp and ~28 traits
glimpse(prey_probable_merge2)

write.csv(prey_probable_merge2, here("data/output_data/prey_trait_probable_select.csv"), 
          row.names = FALSE)
