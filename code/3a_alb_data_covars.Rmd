...
Here we create the key dataframes for analyses, including appending geographic information data back onto our covariate and total diet matrices, then producing subsets of the data for a presence/absence, frequency of occurrence, % abundance and % biomass species community dataframes. Note that the latter datasets do not have enough replication to use in analyses.
...
```{r Load prey traits selection back in, tidy = TRUE}
...
#  dplyr::select(-CiteYear)

alb_covars = read.csv(here::here("data/2_output_data/alb_covars.csv"), check.names = FALSE) #%>%
#  dplyr::select(-CiteYear)

alb_covars_geog = read.csv(here::here("data/2_output_data/alb_covars_longhurst.csv"), check.names = FALSE) #%>%
#  dplyr::select(-CiteYear)
...
#  dplyr::select(-MonthRange) #-optional, 

```

```{r Check on global diet data also}

#check names match
unique(sort(alb_global$StudyID)) == unique(sort(alb_covars$StudyID)) #they do
unique(sort(alb_global$YearEnd)) == unique(sort(alb_covars$YearEnd)) #they do
unique(sort(alb_global$MonthRange)) == unique(sort(alb_covars$MonthRange)) #they do!
unique(sort(alb_global$OceanBasin)) == unique(sort(alb_covars$OceanBasin)) #they do
unique(sort(alb_global$LocatName)) == unique(sort(alb_covars$LocatName)) #they do
```
...
alb_global_diet = alb_global %>%
  ...
  #by = c("StudyID", "OceanBasin", "OceanBasinQ" "YearEnd", "MonthRange")
  dplyr::select(StudyID, CiteAuth, CiteYear, YearEnd, rep_ID, OceanBasin:MonthRange, province, code, lat_cat, pred_life_adjust, stomachs_used:est_ref, prey_class:maxl_type, gape_lmax, gape_height_limit) #system, CiteYear, 

#issue joining data from "Pinkas1971_49"

unique(alb_covars_geog$rep_ID)
unique(alb_global_diet$rep_ID)
as.factor(alb_global_diet$code)
unique(alb_global_diet$code)
summary(as.factor(alb_global_diet$upwell_gyre))
summary(as.factor(alb_global_diet$code))
table(alb_global_diet$code)

#Write over previous output from first data chunk
write.csv(alb_global_diet, here::here("data/2_output_data/alb_global_diet_total_v2.csv"), 
          row.names = FALSE)
```

### Albacore measurement data exploration

Supplementary figures S1a/b:

```{r supplementary data for figures S1, tidy = TRUE}

alb_covars_esti = alb_global_diet %>%
  dplyr::select(StudyID, CiteYear, YearEnd, MonthRange, stomachs_used, OceanBasin, OceanBasinQ, LocatName, LocatLatitude:pred_flest) %>% # 
  distinct(StudyID, CiteYear, OceanBasin, OceanBasinQ, LocatName, LocatLatitude, #CiteYear, 
...
#Save this df for Supplementary Information Section C
#write.csv(alb_covars_esti, here::here("data/2_output_data/....csv"), 
#          row.names = FALSE)
```

```{r supplementary plot for figures S1, tidy = TRUE}

alb_covars_esti_plot <- alb_covars_esti %>%
  mutate(StudyID = fct_reorder(as.factor(StudyID), desc(CiteYear))) %>% #YearEnd
...
ggsave(here::here("outputs_figures/ms_figures/SI_results/figS2_alb_covars_size_esti_plot.jpeg"), 
       plot = alb_covars_esti_plot, width=8, height=8, dpi=300)

```

```{r supplementary data on estimation for SI}

alb_covars_esti_SI = alb_global_diet %>%
  dplyr::select(StudyID, CiteYear, OceanBasin, OceanBasinQ, LocatName, LocatLatitude, LocatLongitude, YearEnd, MonthRange, stomachs_used, 
           pred_life, pred_flmean, pred_flmin, pred_flmax, pred_flmean_est, pred_flest) %>%
  ...
  #       )
#Write over previous output from first data chunk
write.csv(alb_covars_esti_SI, here::here("data/2_output_data/alb_covars_estimates.csv"), 
          row.names = FALSE)

```

```{r albacore size estimation regression, tidy = TRUE}
...
```{r supplementary plot, eval=FALSE}
...
View(alb_global_diet[c(130, 131, 187, 196, 77, 80, 192, 198, 76, 79, 197, 199),])
View(alb_global_diet[c(131, 187, 77, 198, 76, 199),])
...
write.csv(alb_global_dietpa, here::here("data/2_output_data/alb_global_wide_dietpa.csv"), 
          row.names = FALSE)
```

```{r Summary stats PA data}
...
write.csv(alb_global_dietfo, here::here("data/2_output_data/alb_global_wide_dietfo.csv"), 
          row.names = FALSE)
...
write.csv(alb_global_dietn, here::here("data/2_output_data/alb_global_wide_dietn.csv"), row.names = FALSE)
```
...
write.csv(alb_global_dietm, here::here("data/2_output_data/alb_global_wide_dietm.csv"), row.names = FALSE)

```

## EXTRA

If using - Manually add the gyre vs. upwelling / front ecosystems in excel to "alb_covars_longhurst.csv"
