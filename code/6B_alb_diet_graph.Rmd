...
summary(as.factor(index_alb_diet_clust_data$OceanBasin:rep_ID))

table(index_alb_diet_clust_data$rep_ID, index_alb_diet_clust_data$OceanBasin)

write.csv(index_alb_diet_clust_data, here::here("data/output_data/index_alb_diet_clust_data.csv"))
...
```{r Try bar plot S Pacific example}

index_cluster_bar = index_alb_diet_clust_data %>%
  filter(OceanBasin == "S Pacific") %>%
  ggplot() +
  geom_bar(data = index_alb_diet_clust_data, 
             aes(y=relFO, x=YearEnd, #group=rep_ID, 
                 fill=prob.clust.num),
             width = 1, stat = "identity") + #, position = "jitter"
  #geom_jitter(position = position_jitter(width = 0.1, height = 0.1))+
  theme(panel.grid.major = element_blank(),
        axis.title=element_text(size=24),
        axis.text=element_text(size=24),
...
- PROBLEM have redesigned graph and input metric information, but in current graph, half the data disappear... ???
